...
- `statements list`, `statements status`, `statements get`, `statements inspect-membership`, `statements inspect-sync-record`, `statements create`, `statements delete`, `statements reconcile`, `statements unreconcile`, `statements unreconciled`
  ...
  - Typed internal-row replacement fails closed unless atomic transaction sync-blob updates are available. An existing individual Statement sync record is also updated strictly; a legitimately absent one is left absent. Restore rejects selected or unselected members that no longer match the exact forward state, rather than overwriting intervening changes. The restore returns its allocator-generated row ID for replay; these commands are not a generic `--allow-internal` mutation route.
